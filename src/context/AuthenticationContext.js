...
import { getAuthTokensLogin } from '../utils/pre-auth-utils';
import {
...
import { ERROR_TYPES, SECURE_STORE_KEYS, API_CONSTANTS } from '../utils/constants';
...
export function AuthenticationContextProvider({ children }) {
  ...
  const onLogin = async (email, password) => {
    ...
    try {
      const { accessToken, refreshToken, sessionID } = await getAuthTokensLogin(email, password);
  ...
  const onAppOpen = async () => {
    ...
    const refreshTokenData = await getSecureKeys([SECURE_STORE_KEYS.REFRESH_TOKEN]);
